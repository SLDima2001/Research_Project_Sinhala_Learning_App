...
import { View, Text, StyleSheet, TouchableOpacity, Alert, useWindowDimensions, ToastAndroid, Platform, Modal } from 'react-native';
...
export default function StoryScreen() {
    const { id: storyId, title } = useLocalSearchParams();
    ...
    const loadStoryData = async () => {
        if (!storyId) return;
        const data = await getStory(storyId as string);
        if (data) {
            setStory(data);
    ...
    const changeSegment = async (segmentId: string) => {
        ...
        try {
            ...
            if (user?.id) {
                await saveGameScore(user.id, {
                    module: 'storytelling',
                    score: score * 10,
                    maxScore: Math.max(total, 1) * 10,
                    correct: score,
                    total,
                    metadata: { storyId, storyTitle: title },
                    completedAt: new Date().toISOString(),
                });
            }
        } catch (e) {
                ...
                <View style={styles.videoWrapper}>
                    <Video
                        ...
                        onError={(error) => {
                            ...
                            if (Platform.OS === 'android') {
                                ToastAndroid.show("Video Error", ToastAndroid.LONG);
                            }
                        }}
                    />
                </View>
    ...
    return (
        <View style={styles.container}>
            {sceneType === 'narrative' && <NarrativeScene data={scene} onNext={handleLegacyNext} onWordClick={() => { }} />}
